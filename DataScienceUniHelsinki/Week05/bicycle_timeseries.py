...
def bicycle_timeseries():
    cyc_data = split_date_continues()

    cyc_data["Date"] = pd.to_datetime(cyc_data[["Year", "Month", "Day", "Hour"]],
                                      format='%Y%m%d %H:%M')
    print(type(cyc_data["Date"][0]))
    cyc_data = cyc_data.drop(columns=["Year", "Month", "Day", "Hour", "Weekday"])
    cyc_data = cyc_data.set_index("Date")
    print(type(cyc_data.index[0]))
